...
import { SpendyAction, SpendyState } from "./reducer";
import {
    dateDiff,
    formatBankAmount,
...
import { AmazonOrder } from "./amazon";
...
export function UnmatchedAmazon({ state, dispatch }: UnmatchedAmazonProps) {
            ...
            sort(
                unmatchedOrders
                    .map(x => ({
                        ...
                        type: "order",
                        amount: x.total,
                        description: x.items.map(x => x.description).join(","),
                        id: x.orderId,
                    }))
                    .concat(
    ...
    const selection = useMemo(() => {
        ...
        const orders = ids.filter(
            id => unmatchedCombination.find(u => u.id === id)?.type === "order"
        );
        const payments = ids.filter(
            id =>
                unmatchedCombination.find(u => u.id === id)?.type === "payment"
        );
